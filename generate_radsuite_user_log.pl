        ...
        while ( @row = $av_sth->fetchrow_array() )
                {
              	...
              	$batch++;
                Logit_failed ("$row[6]|$row[7]|$row[8]|$row[9]|No User ID found") unless ($row[0]);
                my $counter2 = 0;
                foreach my $items (@row)
                        {
                        $row[$counter2]=$row[$counter2] || "No Value";
                        $counter2++;
                        }
...
sub print_failed_header
	{
	print FAILEDLOG "USERID|LASTNAME|FIRSTNAME|MIDDLENAME|ORGANIZATION|EMAILADDRESS|START|END|LASTACCESSED|IPADDRESS|GROUPNAME|LDAPUSERDESC|LDAPGROUPDESC|LDAPSOURCENAME|LDAPDOMAIN|LDAPDOMAINSUFFIX|LDAPDESC\n";
	}
############################################################################################
#
# Logit Results File
...
sub Open_Database {
  ...
  $dbh = DBI->connect("dbi:Oracle:DARC", "$usr", "$pwd", { PrintError => 1 } ) ;
  do {
